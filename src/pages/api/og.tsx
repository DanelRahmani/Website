...
const generateImage = async (req: NextRequest) => {
  ...
  const siteName = searchParams.get('siteName') || process.env.NEXT_PUBLIC_SITE_NAME || '';
  const avatar =
    searchParams.get('avatar') ||
    `${process.env.NEXT_PUBLIC_URL || ''}/assets/blog/authors/danel.jpeg`;

  const ogUrl = buildOpenGraphUrl({ title, description });
    ...
    (
      <main tw="h-full w-full bg-[#FB2576] pb-1 flex flex-col">
        <div tw="w-full h-full flex flex-col items-start justify-start bg-zinc-900 text-zinc-100 p-8">
          {siteName ? (
            <div tw="self-end text-2xl text-zinc-500">{siteName}</div>
          ) : null}
